...
public:
    ...
    void emit(Op op, Operand dst, Operand a = Operand::none(), Operand b = Operand::none(), Loc loc = {}) {
    ...
    void visit(IndexExpr& n) override {
        ...
        Type elemT; elemT.kind = n.sym->type.elem;
    ...
    void visit(BinOp& n) override {
        ...
        if (n.lhs->type.kind == Type::STRING) {
            if (n.op == BinOp::Eq || n.op == BinOp::Neq) {
                ...
                emit(Op::CALL, t, Operand::func("__rt_streq"), Operand::intC(2), n.loc);
                if (n.op == BinOp::Neq) {
            ...
            if (n.op == BinOp::Add) {
                Operand t = curFunc->newTemp(tyStr());     // codegen allocates 256-byte buffer
                ...
                emit(Op::PARAM, Operand::none(), a,     Operand::none(), n.loc);
                emit(Op::PARAM, Operand::none(), b,     Operand::none(), n.loc);
                emit(Op::CALL, Operand::none(), Operand::func("__rt_strconcat"), Operand::intC(3), n.loc);
                resultExpr = t;
                return;
            }
        }
    ...
    void visit(UnOp& n) override {
        ...
        Operand t = curFunc->newTemp(n.type);
        emit(n.op == UnOp::Neg ? Op::NEG : Op::NOT, t, x, Operand::none(), n.loc);
        resultExpr = t;
    }
    ...
    void visit(CallExpr& n) override {
        ...
        for (auto& a : n.args) args.push_back(genExpr(a.get()));
        for (auto& a : args) emit(Op::PARAM, Operand::none(), a, Operand::none(), n.loc);
        Operand dst = Operand::none();
        if (n.type.kind != Type::VOID_T && !n.type.isError()) {
            dst = curFunc->newTemp(n.type);
        }
        emit(Op::CALL, dst, Operand::func(n.callee), Operand::intC((long long)args.size()), n.loc);
        resultExpr = dst;
    }
    ...
    void genBuiltinCall(CallExpr& n) {
        ...
        if (name == "len" && !n.args.empty() && n.args[0]->type.kind == Type::ARRAY) {
            resultExpr = Operand::intC(n.args[0]->type.arrayLen);
            return;
        }

        // read(int_or_float_var) — runtime returns the value, then we copy into the variable
        if (name == "read" && !n.args.empty()) {
            Type t = n.args[0]->type;
            if (t.kind == Type::INT || t.kind == Type::FLOAT) {
                std::string rt = (t.kind == Type::INT) ? "__rt_read_int" : "__rt_read_float";
        ...
        if (name == "swap")          { byRef[0] = byRef[1] = true; }
        else if (name == "sort" || name == "reverse") byRef[0] = true;
        else if (name == "binary_search") byRef[0] = true;
        else if (name == "read")          byRef[0] = true; // string buffer
        ...
        emit(Op::CALL, dst, Operand::func(rt), Operand::intC((long long)args.size()), n.loc);
        resultExpr = dst;
    }

    Operand genAddrOf(Expr* e, Loc loc) {
        Operand addr = curFunc->newTemp(tyInt()); // pointer = 8 bytes; encode as int operand
        if (auto* vr = dynamic_cast<VarRef*>(e)) {
            ...
            // &arr[idx] — emit base+idx*8 via ADDR + offset; codegen lowers via lea.
            // For simplicity here: spill arr base into a temp, then ADDR-with-offset is a codegen detail.
            // We model this as: ADDR base; then ADD addr, addr, idx*sizeof(elem)
    ...
    static std::string mangleBuiltin(const std::string& name,
                                     const std::vector<std::unique_ptr<Expr>>& args) {
        if (name == "print") {
            Type t = args[0]->type;
            if (t.kind == Type::INT)    return "__rt_print_int";
            if (t.kind == Type::FLOAT)  return "__rt_print_float";
            if (t.kind == Type::STRING) return "__rt_print_str";
            if (t.kind == Type::BOOL)   return "__rt_print_bool";
        }
        if (name == "read") {
            // int/float handled before this; remaining = string
            return "__rt_read_str";
        }
        if (name == "sort")          return "__rt_sort_int";
        if (name == "reverse")       return "__rt_reverse_int";
        if (name == "binary_search") return "__rt_binary_search_int";
        if (name == "gcd")           return "__rt_gcd";
        if (name == "lcm")           return "__rt_lcm";
        if (name == "max" || name == "min" || name == "abs") {
            Type t = args[0]->type;
            const char* suf = (t.kind == Type::INT) ? "int" : "float";
            return "__rt_" + name + "_" + suf;
        }
        if (name == "swap")  return "__rt_swap_i64";
        if (name == "len")   return "__rt_strlen";   // string only by this point
        return name;
    }
    ...
    void visit(DeclStmt& n) override {
        if (n.sym) curFunc->locals.push_back(n.sym);
        if (n.init) {
    ...
    void visit(WhileStmt& n) override {
        ...
        contStack.push(Ltop);
        emit(Op::LABEL, Operand::none(), Operand::label(Ltop), Operand::none(), n.loc);
        Operand cond = genExpr(n.cond.get());
        emit(Op::JZ, Operand::none(), cond, Operand::label(Lend), n.loc);
        n.body->accept(*this);
        emit(Op::JMP, Operand::none(), Operand::label(Ltop), Operand::none(), n.loc);
        emit(Op::LABEL, Operand::none(), Operand::label(Lend), Operand::none(), n.loc);
        breakStack.pop();
        contStack.pop();
    }
    ...
    void visit(ForStmt& n) override {
        ...
        emit(Op::JZ, Operand::none(), cond, Operand::label(Lend), n.loc);
        n.body->accept(*this);
        emit(Op::LABEL, Operand::none(), Operand::label(Lstep), Operand::none(), n.loc);
        n.step->accept(*this);
        emit(Op::JMP, Operand::none(), Operand::label(Ltop), Operand::none(), n.loc);
        emit(Op::LABEL, Operand::none(), Operand::label(Lend), Operand::none(), n.loc);
        breakStack.pop();
    ...
    void visit(ReturnStmt& n) override {
        if (n.value) {
            ...
            emit(Op::RET, Operand::none(), Operand::none(), Operand::none(), n.loc);
        }
    }
    ...
    void visit(BreakStmt& n) override {
        ...
        emit(Op::JMP, Operand::none(), Operand::label(breakStack.top()), Operand::none(), n.loc);
    }
    void visit(ContinueStmt& n) override {
        if (contStack.empty()) return;
        emit(Op::JMP, Operand::none(), Operand::label(contStack.top()), Operand::none(), n.loc);
    }
