...
bool sameTemp(const Operand &a, const Operand &b) {
    return a.kind == Operand::TEMP && b.kind == Operand::TEMP &&
           a.sval == b.sval;
}
bool sameNamed(const Operand &a, const Operand &b) {
    if (a.kind != b.kind)
        return false;
    return (a.kind == Operand::TEMP || a.kind == Operand::VAR) &&
           a.sval == b.sval;
}
...
bool foldPass(IRFunction &f) {
    ...
    for (auto &i : f.code) {
        ...
        if (i.op == Op::NEG && isIntC(i.src1)) {
            makeCopy(i, Operand::intC(-i.src1.ival));
            changed = true;
            continue;
        }
        if (i.op == Op::NEG && isFloatC(i.src1)) {
            makeCopy(i, Operand::floatC(-i.src1.fval));
            changed = true;
            continue;
        }
        if (i.op == Op::NOT && isIntC(i.src1)) {
            makeCopy(i, Operand::boolC(!i.src1.ival));
            changed = true;
            continue;
        }

        // Algebraic identities (int-only — float identities are skipped to
        // preserve IEEE semantics)
        switch (i.op) {
        case Op::ADD:
            if (isIntC(i.src1) && i.src1.ival == 0) {
                makeCopy(i, i.src2);
                changed = true;
            } else if (isIntC(i.src2) && i.src2.ival == 0) {
                makeCopy(i, i.src1);
                changed = true;
            }
            break;
        case Op::SUB:
            if (isIntC(i.src2) && i.src2.ival == 0) {
                makeCopy(i, i.src1);
                changed = true;
            } else if (sameNamed(i.src1, i.src2)) {
                makeCopy(i, Operand::intC(0));
                changed = true;
            }
            break;
        case Op::MUL:
            if ((isIntC(i.src1) && i.src1.ival == 0) ||
                (isIntC(i.src2) && i.src2.ival == 0)) {
                makeCopy(i, Operand::intC(0));
                changed = true;
            } else if (isIntC(i.src1) && i.src1.ival == 1) {
                makeCopy(i, i.src2);
                changed = true;
            } else if (isIntC(i.src2) && i.src2.ival == 1) {
                makeCopy(i, i.src1);
                changed = true;
            }
            break;
        case Op::DIV:
            if (isIntC(i.src2) && i.src2.ival == 1) {
                makeCopy(i, i.src1);
                changed = true;
            }
            break;
        case Op::AND:
            if (isIntC(i.src1) && i.src1.ival == 0) {
                makeCopy(i, Operand::boolC(false));
                changed = true;
            } else if (isIntC(i.src2) && i.src2.ival == 0) {
                makeCopy(i, Operand::boolC(false));
                changed = true;
            ...
            break;
        case Op::OR:
            if (isIntC(i.src1) && i.src1.ival == 1) {
                makeCopy(i, Operand::boolC(true));
                changed = true;
            } else if (isIntC(i.src2) && i.src2.ival == 1) {
                makeCopy(i, Operand::boolC(true));
                changed = true;
            } else if (isIntC(i.src1) && i.src1.ival == 0) {
                makeCopy(i, i.src2);
                changed = true;
            } else if (isIntC(i.src2) && i.src2.ival == 0) {
                makeCopy(i, i.src1);
...
bool propPass(IRFunction &f) {
    ...
    for (auto &i : f.code) {
        ...
        // CALL doesn't modify our local temps (each temp written once
        // typically); leave map.
    }
    return changed;
}
...
bool dcePass(IRFunction &f) {
    ...
    for (const auto &i : f.code) {
        if (i.src1.kind == Operand::TEMP)
            used.insert(i.src1.sval);
        if (i.src2.kind == Operand::TEMP)
            used.insert(i.src2.sval);
        // STORE writes to dst[idx] = val; the array variable is in dst — but
        // STORE has side effects, treat dst as live
    }
    ...
    bool changed = false;
    for (auto &i : f.code) {
        if (isPure(i.op) && i.dst.kind == Operand::TEMP &&
            used.count(i.dst.sval) == 0) {
            changed = true; // drop
        } else {
            kept.push_back(i);
        }
