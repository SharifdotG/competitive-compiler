...
const char* FLOAT_REGS[8] = {"xmm0","xmm1","xmm2","xmm3","xmm4","xmm5","xmm6","xmm7"};

bool isExternalSymbol(const std::string& name) {
    if (name.compare(0, 5, "__rt_") == 0) return true;
    static const std::unordered_set<std::string> libc = {
        "printf","scanf","strcpy","strcat","strcmp","strlen","malloc","free","qsort","puts"
    };
...
std::string offStr(int off) {
    char buf[24];
    if (off < 0) snprintf(buf, sizeof(buf), " - %d", -off);
    else         snprintf(buf, sizeof(buf), " + %d", off);
    return buf;
}

struct FrameInfo {
    std::unordered_map<std::string, int> offsets;     // VAR/TEMP name -> offset from rbp
...
public:
    ...
    void run(const IRProgram& p) {
        ...
        emitTextSection();
        for (const auto& f : p.funcs) emitFunction(f);
    }
...
private:
    ...
    void collectFloatConstants(const IRProgram& p) {
        auto add = [&](const Operand& o) {
            if (o.kind != Operand::FLOAT_CONST) return;
            uint64_t bits;
            std::memcpy(&bits, &o.fval, 8);
            if (floatPoolIndex.count(bits)) return;
            floatPoolIndex[bits] = (int)floatPool.size();
            floatPool.push_back(o.fval);
        };
        for (auto& f : p.funcs)
            for (auto& i : f.code) { add(i.dst); add(i.src1); add(i.src2); }
    }
    ...
    void emitConstantsSection(const IRProgram& p) {
        if (p.stringConstants.empty() && floatPool.empty()) return;
        ...
        for (size_t i = 0; i < p.stringConstants.size(); ++i) {
            ...
            for (char c : p.stringConstants[i]) {
                switch (c) {
                    ...
                    case '"':  fputs("\\\"", out); break;
                    default:
                        if ((unsigned char)c < 32) fprintf(out, "\\%03o", (unsigned char)c);
                        else fputc(c, out);
                        break;
                }
            }
        ...
        for (size_t i = 0; i < floatPool.size(); ++i) {
            ...
            fprintf(out, ".LCF%zu:\n    .quad 0x%016lx   # %g\n",
                    i, (unsigned long)bits, floatPool[i]);
        }
    }
    ...
    void emitFunction(const IRFunction& f) {
        ...
        for (Symbol* p : f.paramSyms) {
            ...
            if (off >= 0) continue;  // came on the stack; already in place
            if (p->type.kind == Type::FLOAT) {
                ...
                fprintf(out, "    mov [rbp%s], %s\n",
                        offStr(off).c_str(), INT_REGS[regI]);
                ++regI;
            }
        }
        ...
        for (size_t k = 0; k < f.code.size(); ++k) {
            ...
            if (i.op == Op::PARAM) { pendingParams.push_back(i.src1); continue; }
            if (i.op == Op::CALL)  { lowerCall(i, pendingParams, fi); pendingParams.clear(); continue; }
            lowerInstr(i, fi);
        }
    ...
    void layoutFrame(const IRFunction& f, FrameInfo& fi) {
        ...
        // Params: first 6 ints in rdi..r9, first 8 floats in xmm0..7, rest on stack at [rbp+16+...]
        int regI = 0, regF = 0;
        int stackParam = 16;
        for (Symbol* p : f.paramSyms) {
            int slot;
            if (p->type.kind == Type::FLOAT) {
                if (regF < 8) { slot = alloc(8); ++regF; }
                else          { slot = stackParam; stackParam += 8; }
            } else {
                if (regI < 6) { slot = alloc(8); ++regI; }
                else          { slot = stackParam; stackParam += 8; }
            }
        ...
        for (Symbol* l : f.locals) {
            int sz = 8;
            if (l->type.kind == Type::ARRAY)        sz = 8 * l->type.arrayLen;
            else if (l->type.kind == Type::STRING) sz = 256;
        ...
        for (auto& i : f.code) {
            auto note = [&](const Operand& o) {
                if (o.kind != Operand::TEMP) return;
                if (fi.offsets.count(o.sval)) return;
                int sz = 8;
                if (o.type.kind == Type::STRING) sz = 256;
                fi.offsets[o.sval] = alloc(sz);
                fi.tempTypes[o.sval] = o.type;
            };
            note(i.dst); note(i.src1); note(i.src2);
        }

        int total = -off;
        if (total % 16 != 0) total += 16 - (total % 16);
        fi.frameSize = total;
    }

    // --------- Operand load / store ---------

    // VAR/TEMP with kind STRING/ARRAY: those slots HOLD the buffer inline, so loading the operand
    // means loading its address (lea). Sema forbids string/array params, so this rule applies uniformly.
    bool varIsByAddress(const Operand& o) {
        if (o.kind != Operand::VAR && o.kind != Operand::TEMP) return false;
        return o.type.kind == Type::ARRAY || o.type.kind == Type::STRING;
    }

    // Load operand into an int-class register (rax/rcx/rdx, or any of INT_REGS).
        ...
        switch (o.kind) {
            ...
            case Operand::VAR:
                fprintf(out, "    movsd %s, qword ptr [rbp%s]\n",
                        xmm, offStr(fi.offsets[o.sval]).c_str());
                return;
            case Operand::FLOAT_CONST: {
                int idx = floatPoolIdx(o.fval);
    ...
    void storeInt(const Operand& o, const char* reg, FrameInfo& fi) {
        if (o.kind != Operand::TEMP && o.kind != Operand::VAR) return;
        ...
        switch (i.op) {
            case Op::ADD: case Op::SUB: case Op::MUL: case Op::DIV: case Op::MOD: {
                if (isFloatOp(i)) {
                    ...
                    const char* op = "addsd";
                    if (i.op == Op::SUB) op = "subsd";
                    else if (i.op == Op::MUL) op = "mulsd";
                    else if (i.op == Op::DIV) op = "divsd";
                    else if (i.op == Op::MOD) { /* unsupported for floats; sema disallows */ return; }
                ...
                switch (i.op) {
                    case Op::ADD: fprintf(out, "    add rax, rcx\n"); break;
                    case Op::SUB: fprintf(out, "    sub rax, rcx\n"); break;
                    case Op::MUL: fprintf(out, "    imul rax, rcx\n"); break;
                    case Op::DIV: fprintf(out, "    cqo\n    idiv rcx\n"); break;
                    case Op::MOD: fprintf(out, "    cqo\n    idiv rcx\n    mov rax, rdx\n"); break;
                    default: break;
                }
            ...
            case Op::LT: case Op::LE: case Op::GT: case Op::GE: {
                ...
                switch (i.op) {
                    case Op::EQ:  setOp = "sete";  break;
                    case Op::NEQ: setOp = "setne"; break;
                    case Op::LT:  setOp = isFloat ? "setb"  : "setl";  break;
                    case Op::LE:  setOp = isFloat ? "setbe" : "setle"; break;
                    case Op::GT:  setOp = isFloat ? "seta"  : "setg";  break;
                    case Op::GE:  setOp = isFloat ? "setae" : "setge"; break;
                    default: break;
                }
                fprintf(out, "    %s al\n    movzx rax, al\n", setOp);
                storeInt(i.dst, "rax", fi);
            ...
            case Op::RET:
                if (!i.src1.isNone()) {
                    if (i.src1.type.kind == Type::FLOAT) loadFloat(i.src1, "xmm0", fi);
                    else                                  loadInt(i.src1, "rax", fi);
                }
                fprintf(out, "    mov rsp, rbp\n    pop rbp\n    ret\n");
                return;
            default:
    ...
    void lowerCall(const Instr& callInstr, std::vector<Operand>& params, FrameInfo& fi) {
        // Classify each param: float -> xmm reg, int/ptr -> int reg, overflow -> stack.
        std::vector<int> intRegArgs, floatRegArgs, stackArgs;
        for (size_t k = 0; k < params.size(); ++k) {
            Operand& p = params[k];
            if (p.type.kind == Type::FLOAT) {
                if (floatRegArgs.size() < 8) floatRegArgs.push_back(k);
                else                          stackArgs.push_back(k);
            } else {
                if (intRegArgs.size() < 6) intRegArgs.push_back(k);
                else                        stackArgs.push_back(k);
            }
        }
        ...
        if (padding) fprintf(out, "    sub rsp, 8\n");
        ...
        // For variadic targets, al = number of xmm registers used. Set it always — harmless otherwise.
        ...
        if (unwind > 0) fprintf(out, "    add rsp, %d\n", unwind);
