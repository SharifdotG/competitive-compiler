...
SymbolTable::SymbolTable() {
    // No global scope by default; sema enters one before processing the
    // program.
}
...
Symbol *SymbolTable::declare(const std::string &name, const Type &t, Loc loc,
                             bool isParam) {
    Scope *cur = current();
    if (!cur)
        return nullptr;
    if (cur->table.count(name))
        return nullptr;
...
FuncSymbol *SymbolTable::declareFunc(const std::string &name, Type ret,
                                     std::vector<Type> params, bool isBuiltin,
                                     Loc loc) {
