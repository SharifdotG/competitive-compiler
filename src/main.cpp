...
static int dump_tokens(const char* path) {
    ...
    while ((t = yylex()) != 0) {
        printf("%3d:%-3d  %-12s  ",
               yylloc.first_line, yylloc.first_column, tok_name(t));
        if (t == STRING_LIT)      printf("\"%s\"\n", yylval.sval);
        else if (t == INT_LIT)    printf("%lld\n", yylval.ival);
        else if (t == FLOAT_LIT)  printf("%g\n",   yylval.fval);
        else if (t == IDENT)      printf("%s\n",   yylval.sval);
        else                      printf("%s\n",   yytext);
    }
    fclose(yyin);
    return 0;
...
int main(int argc, char** argv) {
    enum class Mode { Compile, Tokens, Ast, Ir, OptIr, Asm } mode = Mode::Compile;
    ...
    for (int i = 1; i < argc; ++i) {
        std::string a = argv[i];
        if      (a == "--tokens")  mode = Mode::Tokens;
        else if (a == "--ast")     mode = Mode::Ast;
        else if (a == "--ir")      mode = Mode::Ir;
        else if (a == "--opt-ir")  mode = Mode::OptIr;
        else if (a == "-S")        mode = Mode::Asm;
    ...
    // Always optimize before codegen (the optimizer is conservative and only improves things)
    optimizeIR(*ir);
    ...
    if (mode == Mode::Asm) {
        ...
        if (!f) { perror(asmPath.c_str()); delete ir; return 1; }
        emitX86Asm(*ir, f);
        fclose(f);
        delete ir;
        return 0;
