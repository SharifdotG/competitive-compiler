...
static void printOperand(FILE *out, const Operand &o) {
    ...
    case Operand::TEMP:
    case Operand::VAR:
        fputs(o.sval.c_str(), out);
        break;
    case Operand::INT_CONST:
        fprintf(out, "%lld", o.ival);
        break;
    case Operand::FLOAT_CONST:
        fprintf(out, "%g", o.fval);
        break;
    case Operand::STR_CONST:
        fprintf(out, "@str%lld", o.ival);
        break;
...
static void printInstr(FILE *out, const Instr &i) {
    ...
    case Op::OR:
        fputs("    ", out);
        printOperand(out, i.dst);
        fputs(" = ", out);
        printOperand(out, i.src1);
        fprintf(out, " %s ", opSym(i.op));
...
void printIR(const IRProgram &p, FILE *out) {
    ...
    for (const auto &f : p.funcs) {
        ...
        for (size_t i = 0; i < f.paramNames.size(); ++i) {
            if (i)
                fputs(", ", out);
            fprintf(out, "%s: %s", f.paramNames[i].c_str(),
                    typeStr(f.paramTypes[i]));
        }
