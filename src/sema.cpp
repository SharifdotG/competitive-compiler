...
public:
    ...
    void visit(IndexExpr& n) override {
        ...
        if (s->type.kind != Type::ARRAY) {
            error(n.loc, "'" + n.name + "' is not an array (it is " + typeName(s->type) + ")");
            n.type = tyError();
            return;
        }
        if (!n.idx->type.isError() && n.idx->type.kind != Type::INT) {
            error(n.idx->loc, std::string("array index must be int, got ") + typeName(n.idx->type));
        }
        Type elem;
    ...
    void visit(CallExpr& n) override {
        ...
        if (fs->paramTypes.size() != n.args.size()) {
            ...
            for (size_t i = 0; i < n.args.size(); ++i) {
                ...
                if (at.isError()) continue;
                if (pt != at) {
                    char buf[160];
                    snprintf(buf, sizeof(buf),
                        "argument %zu of '%s': expected %s, got %s",
                        i + 1, n.callee.c_str(), typeName(pt), typeName(at));
        ...
        switch (n.op) {
            case BinOp::Add:
                if (lt.kind == Type::STRING && rt.kind == Type::STRING) {
                    ...
                    error(n.loc, std::string("operator + requires same numeric type or string, got ")
                        + typeName(lt) + " and " + typeName(rt));
                    n.type = tyError();
                }
                return;
            case BinOp::Sub: case BinOp::Mul: case BinOp::Div:
                if (lt.isNumeric() && rt.isNumeric() && lt == rt) {
                    n.type = lt;
                } else {
                    error(n.loc, std::string("operator ") + binopName(n.op) +
                        " requires same numeric type, got " + typeName(lt) + " and " + typeName(rt));
                    n.type = tyError();
                }
                return;
            case BinOp::Mod:
                if (lt.kind == Type::INT && rt.kind == Type::INT) n.type = tyInt();
                else { error(n.loc, "operator % requires int operands"); n.type = tyError(); }
                return;
            case BinOp::Eq: case BinOp::Neq:
                if (lt == rt) n.type = tyBool();
                else { error(n.loc, std::string("operator ") + binopName(n.op) +
                            " requires same type, got " + typeName(lt) + " and " + typeName(rt));
                       n.type = tyError(); }
                return;
            case BinOp::Lt: case BinOp::Le: case BinOp::Gt: case BinOp::Ge:
                if (lt.isNumeric() && rt.isNumeric() && lt == rt) n.type = tyBool();
                else { error(n.loc, std::string("operator ") + binopName(n.op) +
                            " requires same numeric type"); n.type = tyError(); }
                return;
            case BinOp::And: case BinOp::Or:
                if (lt.kind == Type::BOOL && rt.kind == Type::BOOL) n.type = tyBool();
                else { error(n.loc, std::string("operator ") + binopName(n.op) +
                            " requires bool operands"); n.type = tyError(); }
                return;
        }
    }
    ...
    void visit(UnOp& n) override {
        ...
        if (n.operand->type.isError()) { n.type = tyError(); return; }
        if (n.op == UnOp::Neg) {
            if (n.operand->type.isNumeric()) n.type = n.operand->type;
            else { error(n.loc, "unary - requires numeric operand"); n.type = tyError(); }
        } else { // Not
            if (n.operand->type.kind == Type::BOOL) n.type = tyBool();
            else { error(n.loc, "unary ! requires bool operand"); n.type = tyError(); }
        }
    }
    ...
    void visit(DeclStmt& n) override {
        ...
        if (n.init) {
            ...
            } else if (!n.init->type.isError() && n.init->type != n.declaredType) {
                error(n.loc, std::string("initializer type ") + typeName(n.init->type) +
                    " does not match declared type " + typeName(n.declaredType));
            }
        }
    }
    ...
    void visit(AssignStmt& n) override {
        ...
        if (n.target->type.isError() || n.value->type.isError()) return;
        if (n.target->type.kind == Type::ARRAY) {
            error(n.loc, "cannot assign to entire array");
            return;
        }
        if (n.target->type != n.value->type) {
            error(n.loc, std::string("type mismatch in assignment: target is ") +
                typeName(n.target->type) + ", value is " + typeName(n.value->type));
            return;
        }
    ...
    void visit(IfStmt& n) override {
        ...
        if (!n.cond->type.isError() && n.cond->type.kind != Type::BOOL) {
            error(n.cond->loc, std::string("if condition must be bool, got ") + typeName(n.cond->type));
        }
        n.thenS->accept(*this);
        if (n.elseS) n.elseS->accept(*this);
    }

    void visit(WhileStmt& n) override {
        n.cond->accept(*this);
        if (!n.cond->type.isError() && n.cond->type.kind != Type::BOOL) {
            error(n.cond->loc, std::string("while condition must be bool, got ") + typeName(n.cond->type));
        }
        ++loopDepth;
        n.body->accept(*this);
        --loopDepth;
    ...
    void visit(ReturnStmt& n) override {
        if (n.value) {
            n.value->accept(*this);
            if (!n.value->type.isError() && n.value->type != currentReturnType) {
                error(n.loc, std::string("return type mismatch: expected ") +
                    typeName(currentReturnType) + ", got " + typeName(n.value->type));
            }
        } else {
            if (currentReturnType.kind != Type::VOID_T) {
                error(n.loc, std::string("function must return ") + typeName(currentReturnType));
            }
        }
    }
    ...
    void visit(BreakStmt& n) override {
        if (loopDepth == 0) error(n.loc, "break outside loop");
    }
    void visit(ContinueStmt& n) override {
        if (loopDepth == 0) error(n.loc, "continue outside loop");
    }
    ...
    void visit(FuncDecl& n) override {
        ...
        for (auto& p : n.params) {
            if (p.type.kind == Type::STRING) {
                error(p.loc, "string parameters are not supported in v1 (locals only)");
            }
            if (p.type.kind == Type::ARRAY) {
                error(p.loc, "array parameters are not supported in v1 (locals only)");
            }
        ...
        if (n.body) {
            // Inline the body's statements at the function scope so params + body share scope.
            for (auto& s : n.body->stmts) s->accept(*this);
        }
        symtab.exitScope();

        if (n.returnType.kind != Type::VOID_T && n.body && !endsInReturn(n.body.get())) {
            error(n.loc, "function '" + n.name + "' may not return on all paths");
        }
    }
    ...
    void visit(Program& n) override {
        ...
        for (auto& f : n.funcs) {
            ...
            FuncSymbol* fs = symtab.declareFunc(f->name, f->returnType, ptypes, /*isBuiltin=*/false, f->loc);
            if (!fs) error(f->loc, "duplicate function '" + f->name + "'");
            else f->sym = fs;
        }
...
private:
    ...
    static bool endsInReturn(Stmt* s) {
        ...
        if (auto* i = dynamic_cast<IfStmt*>(s)) {
            return i->elseS && endsInReturn(i->thenS.get()) && endsInReturn(i->elseS.get());
        }
        return false;
    }
    ...
    void installBuiltins() {
        ...
        Type voidT; voidT.kind = Type::VOID_T;
        // print/read are dispatched specially — declare with empty params.
        reg("print", voidT, {});
        reg("read",  voidT, {});
        // sort/reverse/binary_search dispatched specially (need array first arg).
        reg("sort",          voidT, {});
        reg("reverse",       voidT, {});
    ...
    void checkBuiltinCall(CallExpr& n) {
        ...
        if (name == "print") {
            if (n.args.size() != 1) { error(n.loc, "print() takes exactly 1 argument"); }
            else {
                Type t = n.args[0]->type;
                if (!t.isError() && t.kind != Type::INT && t.kind != Type::FLOAT &&
                    t.kind != Type::STRING && t.kind != Type::BOOL) {
                    error(n.args[0]->loc, std::string("print() supports int/float/string/bool, got ") + typeName(t));
                }
            }
            n.type = tyVoid();
            return;
        ...
        if (name == "read") {
            if (n.args.size() != 1) { error(n.loc, "read() takes exactly 1 argument"); }
            else {
                Expr* a = n.args[0].get();
                if (!isLValueExpr(a)) {
                    error(a->loc, "read() argument must be a variable or array element");
                } else {
                    Type t = a->type;
                    if (!t.isError() && t.kind != Type::INT && t.kind != Type::FLOAT && t.kind != Type::STRING) {
                        error(a->loc, std::string("read() supports int/float/string, got ") + typeName(t));
                    }
                }
            }
            n.type = tyVoid();
        ...
        if (name == "sort" || name == "reverse") {
            if (n.args.size() != 2) {
                ...
                if (!at.isError() && at.kind != Type::ARRAY) {
                    error(n.args[0]->loc, name + "() first argument must be an array");
                } else if (!at.isError() && at.elem != Type::INT) {
                    error(n.args[0]->loc, name + "() currently supports int arrays only");
                }
        ...
        if (name == "binary_search") {
            if (n.args.size() != 3) {
                error(n.loc, "binary_search() takes 3 arguments: array, length, key");
            } else {
                Type at = n.args[0]->type, lt = n.args[1]->type, kt = n.args[2]->type;
                if (!at.isError() && (at.kind != Type::ARRAY || at.elem != Type::INT)) {
        ...
        if (name == "max" || name == "min") {
            if (n.args.size() != 2) {
                ...
                if (a.isError() || b.isError()) { n.type = tyError(); return; }
                if (!a.isNumeric() || !b.isNumeric() || a != b) {
                    error(n.loc, name + "() requires two numeric operands of the same type");
                    n.type = tyError();
                } else {
                    n.type = a;
                }
        ...
        if (name == "abs") {
            if (n.args.size() != 1) {
                ...
                if (a.isError()) { n.type = tyError(); return; }
                if (!a.isNumeric()) {
            ...
            if (n.args.size() != 2) {
                ...
                for (int i = 0; i < 2; ++i) {
                    if (!isLValueExpr(n.args[i].get())) {
                        error(n.args[i]->loc, "swap() argument must be a variable or array element");
                    }
                }
                if (!n.args[0]->type.isError() && !n.args[1]->type.isError() &&
                    n.args[0]->type != n.args[1]->type) {
                    error(n.loc, "swap() requires both arguments to have the same type");
                }
                if (!n.args[0]->type.isError() && n.args[0]->type.kind == Type::ARRAY) {
        ...
        if (name == "len") {
            if (n.args.size() != 1) {
                ...
                Type a = n.args[0]->type;
                if (!a.isError() && a.kind != Type::ARRAY && a.kind != Type::STRING) {
                    error(n.args[0]->loc, std::string("len() requires array or string, got ") + typeName(a));
                }
            }
            n.type = tyInt();
            return;
        ...
        if (name == "gcd" || name == "lcm") {
            // Fixed signature already checked above? No — we marked them as builtin so we get here.
            if (n.args.size() != 2) {
                error(n.loc, name + "() takes 2 arguments");
            } else {
                for (int i = 0; i < 2; ++i) {
                    ...
                    if (!t.isError() && t.kind != Type::INT) {
                        error(n.args[i]->loc, name + "() requires int arguments");
                    }
                }
            }
            n.type = tyInt();
    ...
    static Type tyVoid() { Type t; t.kind = Type::VOID_T; return t; }
};
