...
struct Operand {
    ...
    std::string
        sval; // VAR/TEMP/LABEL/FUNC name; STR_CONST raw text (informational)
    Type type;

    static Operand none() { return {}; }
    static Operand intC(long long v) {
        Operand o;
        o.kind = INT_CONST;
        o.ival = v;
        o.type = tyInt();
    ...
    static Operand floatC(double v) {
        Operand o;
        o.kind = FLOAT_CONST;
        o.fval = v;
        o.type = tyFloat();
...
struct IRFunction {
    ...
    std::vector<Symbol *>
        locals; // collected as DeclStmts are visited (for codegen frame layout)
