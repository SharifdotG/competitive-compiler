/* runtime.c — built-in implementations linked into every competitive-lang executable. */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* read(string): caller passes a fixed 256-byte buffer (allocated by codegen for string locals). */
...
void __rt_sort_int(long long* a, long long n) {
    if (n > 1) qsort(a, (size_t)n, sizeof(long long), __rt_cmp_ll);
}
...
/* Concat into a caller-supplied 256-byte destination buffer. Truncates if too long. */
void __rt_strconcat(char* dst, const char* a, const char* b) {
    size_t la = strlen(a), lb = strlen(b);
    if (la > 255) la = 255;
    if (la + lb > 255) lb = 255 - la;
    memcpy(dst, a, la);
    memcpy(dst + la, b, lb);
    dst[la + lb] = '\0';
}
